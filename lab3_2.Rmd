...
## gather()
problem with data is that a, b, c, d are not column names.the column name is drugs.A, B, c, D are variables.so we need to take that data of what margaret is like for A, what Hawkguy is like for A, what trapper is like for A...ect. Each row needs to have 1 observation per drug. also R is based on vectors. a character and number cannot mix
...
#not tidy bc patient includes 2 observations: name and sex
```{r}
...
```{r}
#assign new names
#assign to neew object to not loose renames
tidy3 <-
  tidy2 %>% 
  dplyr::rename(MASH_character = patient, Sex = sex, Drug =
                  drug, Heartrate_bpm = heartrate)
tidy3
```
...
tidyFaces <- faces %>% 
  ...
  gather(Face_1, Face_2, Face_3, key="Faces", value="identification")
tidyFaces
```
